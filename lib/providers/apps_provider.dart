// Manages state related to the list of Apps tracked by CryptApps,
...
import 'package:CryptApps/components/generated_form_modal.dart';
import 'package:CryptApps/custom_errors.dart';
import 'package:CryptApps/main.dart';
import 'package:CryptApps/providers/logs_provider.dart';
import 'package:CryptApps/providers/notifications_provider.dart';
...
class AppsProvider with ChangeNotifier {
  ...
  Future<File> handleAPKIDChange(App app, PackageInfo? newInfo,
    ...
    if (newInfo != null) {
      ...
      throw CryptAppsError('Could not get ID from APK');
    }
    return downloadedFile;
  }
  ...
  Future<bool> canInstallSilently(App app) async {
    ...
    if (installerPackageName != CryptAppsId) {
  ...
  Future<List<String>> downloadAndInstallLatestApps(
    ...
    for (var id in appIds) {
      if (apps[id] == null) {
        throw CryptAppsError(tr('appNotFound'));
      }
      MapEntry<String, String>? apkUrl;
    ...
    // Move CryptApps to the end of the line (let all other apps update first)
    appsToInstall =
        moveStrToEnd(appsToInstall, CryptAppsId, strB: CryptAppsTempId);
    ...
    for (var id in appsToInstall) {
      try {
        ...
        if (!(await settingsProvider.getInstallPermission(enforce: false))) {
          throw CryptAppsError(tr('cancelled'));
        }
                ...
                : [
                    [
                      GeneratedFormSwitch('rmAppEntry',
                          label: tr('removeFromCryptApps'), defaultValue: true)
                    ],
                    [
  ...
  Future<String?> exportApps(
    ...
    if (!pickOnly) {
      var result = await saf.createFile(exportDir,
          displayName:
              '${tr('CryptAppsExportHyphenatedLowercase')}-${DateTime.now().toIso8601String().replaceAll(':', '-')}${isAuto ? '-auto' : ''}.json',
          mimeType: 'application/json',
          bytes: Uint8List.fromList(utf8.encode(
              jsonEncode(apps.values.map((e) => e.app.toJson()).toList()))));
      if (result == null) {
        throw CryptAppsError(tr('unexpectedError'));
      }
      returnPath =
...
Future<void> bgUpdateCheck(int taskId, Map<String, dynamic>? params) async {
  ...
  if (!installMode) {
    ...
    var tempObtArr = toInstall.where((element) => element.key == CryptAppsId);
    if (tempObtArr.isNotEmpty) {
      // Move CryptApps to the end of the list as it must always install last
